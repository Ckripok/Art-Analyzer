...
if __name__ == "__main__":
    DATASET_DIR = '../../dataset_genre' # меняем на датасет
    BATCH_SIZE = 64
    EPOCHS = 10
    LR = 1e-4
    IMG_SIZE = 224
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Validation Accuracy: {acc:.4f} | Val Loss: {val_loss / len(val_loader):.4f}")
        if acc > best_acc:
            best_acc = acc
            torch.save(model.state_dict(), '../../models/model_genre.pth') # путь
